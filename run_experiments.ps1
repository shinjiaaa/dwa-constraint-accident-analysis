# PowerShell 스크립트: S1과 S2 실험 각 10회 실행
...
for ($seed = 0; $seed -lt 10; $seed++) {
    ...
    wsl bash -c "cd $projectDir && source ~/venvs/dasc/bin/activate && python scripts/run_scenario.py --scenario s1 --seed $seed --out outputs/ --gui"
}

Write-Host "`n=== Running S2 experiments (10 runs) ===" -ForegroundColor Cyan
for ($seed = 0; $seed -lt 10; $seed++) {
    Write-Host "Running S2 seed $seed..." -ForegroundColor Yellow
    wsl bash -c "cd $projectDir && source ~/venvs/dasc/bin/activate && python scripts/run_scenario.py --scenario s2 --seed $seed --out outputs/ --gui"
}
